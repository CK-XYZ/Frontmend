...
- Current local application source: `49f7733`; adds the honest diagnostic blocker and resumable blocker history to persisted Assess missions, focused priorities, the four-stage diagnostic chain, contextual assessment receipt, explicit repair preparation, and the visible authority boundary; this candidate remains local-only
...
8. If `assessmentComplete` is false, ChatGPT must follow the exact `nextAction` and open the supported diagnostic mission. If it cannot access the owning repository or reproduce the same runtime, confirm the contextual set contains `record_diagnostic_blocker`; call it with the mission ID, one exact enum reason (`browser-unavailable`, `repository-unavailable`, `not-reproduced`, `wrong-repository`, or `conflicting-runtime`), and a short non-sensitive summary. The result must say `state: blocked`, `assessmentComplete: false`, retain the measured stage, leave browser/repository/check stages **Required**, and return no repair authority. The page must show **Assessment blocked · evidence retained** and the same blocker. `record_diagnostic_blocker`, `get_assessment_receipt`, staging, and **Export assessment** must then be absent; `submit_runtime_diagnosis` remains available only as the recovery path for a later capable session. Do not fabricate ownership or stop silently at Lighthouse.
9. If `assessmentComplete` is true, confirm `get_assessment_receipt` appears without reloading stale tools. Call it with `{}` and compare its audit ID, retained focus, priorities, measured versus contributed provenance, and all-false repair/implementation/deployment/resolution authority with the visible **Export assessment** Markdown download. For a zero-priority result, require an honest zero count rather than invented work.
10. Confirm the opening audit prompt did not call `prepare_site_repair` or `stage_site_repair`. Then send:

   > Please prepare the first priority for a fix, but don't approve, change, or deploy anything.

11. Confirm `prepare_site_repair` freezes that finding, the visible intent changes to **Preparing a fix**, and the result says recorded intent only. It must not create approval, consume auto allowance, claim an edit, deploy, or attest deployment. Staging appears only if any required diagnosis is complete.
12. Call `get_site_audit_results` with `{ "unexpected": true }`. It must return structured `INVALID_INPUT` and leave visible mission state unchanged. Inspect **Agent log** and confirm entries contain no prompts, target URL, arguments, patches, or secrets.
13. Save screenshots of landing discovery, Assess focus, running tools, completed measurement versus assessment state, receipt appearance or honest absence, diagnostic continuation or real blocker, explicit preparation, Recently used/Sources, and Agent log.

Failure conditions: **Human mode**; missing or stale tools; focus lost on the empty result call; more than three priorities; stopping after Lighthouse while a valid next action exists; narrating a blocker without persisting it; fabricated repository evidence; a blocked mission exporting a receipt or unlocking staging; repair preparation during the audit-only prompt; staging without explicit preparation; agent approval/deployment/attestation; or visible and structured state disagreement.
...
5. For a console-error, contrast-node, or main-thread-blocking finding, confirm the contextual tool set exposes `open_diagnostic_mission` but withholds both `get_assessment_receipt` and agent repair staging. The initial visible and structured `evidenceChain` must retain provider measurement while marking browser reproduction, repository ownership, and planned checks as **Required**. The agent must reproduce the issue in the browser, inspect the owning repository code, and call `submit_runtime_diagnosis` with bounded observations, repository-relative locations, and planned checks. Confirm those three stages become **Contributed** in both the UI and WebMCP result, the Lighthouse symptom remains **measured**, and the causal diagnosis remains **agent-reported**. Do not accept a diagnosis inferred from Lighthouse alone. If this supposedly repository-capable session cannot access or reconcile the correct source, require `record_diagnostic_blocker` and stop the repair path; that is a truthful failed demo prerequisite, not assessment completion.
6. Refresh the contextual capability view. Confirm `get_assessment_receipt` and **Export assessment** appear only after `assessmentComplete` becomes true. Call the tool with `{}`; compare the structured receipt with the downloaded Markdown, including the audit ID, retained mission, ranked provider evidence, separate agent-reported contribution, and explicit false authority for source receipt, repair approval, implementation, deployment, and resolution.
...
10. If a structured diagnostic finding exists, invoke `open_diagnostic_mission` with its `findingId`; confirm both `submit_runtime_diagnosis` and `record_diagnostic_blocker` appear while `get_assessment_receipt` and staging remain absent. Because this Inspector session has no repository evidence, invoke `record_diagnostic_blocker` with `{ "missionId": "<returned-id>", "reason": "repository-unavailable", "summary": "This isolated Chrome verification profile has no authorised access to the repository that owns the deployed bundle." }`. Confirm the result and page agree on `blocked`, measured evidence remains retained, the three missing stages remain required, receipt/staging remain absent, and only `submit_runtime_diagnosis` remains as a recovery capability. Do not invent repository evidence. If the retained assessment is already complete from measured evidence and no diagnostic mission is required, confirm `get_assessment_receipt` appears, call it with `{}`, and compare its structured data with the visible **Export assessment** download and authority boundary.
11. Only for an assessment that is genuinely complete, invoke `prepare_site_repair` for one retained priority and confirm the visible **Preparing a fix** state; `stage_site_repair` may appear only for a measured-sufficient or fully diagnosed selected finding.
12. Negative-schema check: call `get_site_audit_results` with `{ "unexpected": true }`. It must fail with a structured validation error and must not change visible state.
13. Close the tab, open `FRONTMEND_URL` in a new tab, and confirm only `start_site_audit` is registered again. This proves page-scoped lifecycle cleanup rather than stale tool retention.
14. Save screenshots of the Chrome version, enabled flag, console capability check, Inspector tool lists at landing/running/completed/blocked states, the blocker card with retained evidence chain, receipt appearance or honest absence, structured result comparison, negative-schema error, visible Agent log, and explicit preparation/repair gate.
...
The deployed version may be labelled **RC3 deployed, HTTP/API-verified, production-Lighthouse-verified, and currently publicly reachable**. The current local mission candidate remains undeployed and cannot inherit that live proof. Neither may be labelled current-version Chrome-smoke-verified, ChatGPT-WebMCP-verified, Chrome-WebMCP-verified, or submission-ready until the procedures above have genuine receipts. The deployed application source is `a20e1ff`; current application source `49f7733` and this newer judge documentation have not been pushed to a public remote or deployed. Keep the temporary public bypass active throughout judge testing unless it is deliberately replaced by another verified public-access arrangement.
