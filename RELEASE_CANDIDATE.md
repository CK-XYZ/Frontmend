...
8. If `assessmentComplete` is false, ChatGPT must follow the exact `nextAction`. It should open a supported diagnostic mission and contribute only evidence it can actually obtain. Without repository access, it must name that blocker after opening/reading the mission rather than fabricate ownership or stop at Lighthouse. Confirm `get_assessment_receipt` and the visible **Export assessment** action remain absent while this obligation is incomplete.
...
14. Save screenshots of the Chrome version, enabled flag, console capability check, Inspector tool lists at landing/running/completed states, receipt appearance or honest absence, structured result comparison, negative-schema error, visible Agent log, and explicit preparation/repair gate.
