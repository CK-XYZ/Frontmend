...
- Live PageSpeed Insights/Lighthouse evidence for mobile and desktop, including bounded screenshots when supplied by the provider.
- A truthful live-document fallback when Lighthouse is unavailable: bounded public HTML and response headers are inspected without claiming a browser capture or viewport measurement, while a structured Document profile records HTML size, element counts, inline code, external origins, and observed security headers.
...
- A shared repair-mission state machine that assigns measure, draft, review, deploy, and verify steps to Frontmend, the agent/person, or the external site owner and exposes allowed next actions through WebMCP.
...
- A library of twelve WebMCP tools covering audit start/progress/cancellation/results, single-route and multi-page exploration, verification receipts, repair staging/revision/workspace, and verification; only the capabilities valid for the visible page state are registered at any moment.
...
The current slice diagnoses a deliberately narrow set of Lighthouse and document rules. A repair artifact is a reviewed proposal: Frontmend does not claim it edited a third-party site. The person deploys the change through their normal source and hosting workflow, then Frontmend performs a fresh public audit. A successful local run, build, or Wrangler dry run is not a deployment or production proof.
...
`bun test` is clean-tree safe: the packaging contract always rebuilds the current Vite/Sites artifact before asserting the emitted files. `bun run test` runs the same 84 contracts.
