...
- An optional repository implementation receipt after human approval: the coding agent may report only repository-relative filenames, check outcomes, a short summary, and an optional Git object ID. Repeated reports retain a bounded five-receipt history, so a later pass cannot silently erase an earlier failure; the latest receipt is frozen into subsequent verification proof, while remaining explicitly agent-reported rather than source, check, deployment, or resolution evidence.
