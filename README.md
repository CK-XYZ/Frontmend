...
- Browser-observed issues carry their own provenance, become ranked assessment priorities, and can continue through the same repository fix-brief, diagnosis, repair, and fresh-verification contracts as provider findings.
...
- An honest diagnostic-blocker outcome lets an agent record a bounded unavailable-browser, unavailable/wrong-repository, non-reproduction, or conflicting-runtime reason instead of fabricating ownership or leaving the mission silently hanging. The visible assessment remains blocked, measured evidence stays unresolved, receipt and repair staging remain unavailable, and a later capable agent can replace the blocker with real diagnosis while its history is retained.
- A portable assessment receipt appears only after the retained mission is genuinely complete. Its structured WebMCP result and Markdown download freeze ranked provider evidence beside separately attributed browser, repository, and planned-check contributions while explicitly proving no repair, implementation, deployment, or resolution.
...
- A shared repair-mission state machine that assigns measure, draft, review/delegation, implement, deploy, and verify steps to Frontmend, the coding agent/person, or the external site owner; it exposes the same allowed next actions and the exact approval evidence through WebMCP.
...
| `open_browser_review` | Open the persisted rendered-browser review required by an agent-started accessibility or SEO assessment. |
| `record_browser_review_check` | Record the current exact browser check as passed, issue, or honestly blocked, with separately attributed observations and bounded findings. |
...
`bun test` is clean-tree safe: the packaging contract always rebuilds the current Vite/Sites artifact before asserting the emitted files. `bun test` currently runs 147 contracts.
