...
- [x] **30. Bind continuation responses to their authoritative workspace**
  What was built: The shared service now validates audit results, checkpoints, aggregate verification, browser review, diagnosis, repair, route, cancellation, and verification responses against the requested audit and retained record before publication. Related-route starts validate the requested path and parent audit. Finding-led diagnosis and repair, rendered-review focus/check acknowledgement, and policy changes also validate the exact requested action. The Worker now returns the same parent mission checkpoint as local runtime, while aggregate repair proof identifies its root audit.
  Acceptance: A foreign audit, checkpoint, review ID, diagnostic mission ID, repair ID, route parent/path, verification baseline, primary assignment, finding, review scope/check, or policy mode cannot alter the active workspace or any shared cache; delayed responses are still identity-checked after reset; Worker/local related-route payloads agree; idempotent repair intent retains its original attribution; valid Human and WebMCP transitions, the twenty-one tools, and every authority boundary remain unchanged.
  Verify: `bun test tests/audit-service.test.mjs tests/verification-impact-contract.test.mjs tests/sites-worker.test.mjs`; `bun test`; `bun run build`; `bunx wrangler deploy --dry-run --strict --config wrangler.jsonc`; `git diff --check`.

- [x] **31. Require checkpoint-complete, revision-monotonic mission publication**
  What was built: Every continuation mutation and direct results read now requires the authoritative mission checkpoint before its report, review, diagnosis, repair, policy, exploration, route, cancellation, or verification state can enter the shared cache. Missing, malformed, foreign, or older checkpoints fail through the existing safe response boundary. Worker and local workspace reads expose the same current checkpoint.
  Acceptance: Omitting a checkpoint cannot bypass audit identity or revision checks; a response older than the retained mission cannot regress state; failed validation leaves every existing cache entry untouched; audit start remains the only mutation exemption; idempotent behaviour, Human-only fallback, local/Worker parity, all authority boundaries, and the twenty-one WebMCP tools remain unchanged.
  Verify: `bun test tests/audit-service.test.mjs`; `bun test tests/audit-service.test.mjs tests/sites-worker.test.mjs tests/webmcp.test.mjs`; `bun test`; `bun run build`; `bunx wrangler deploy --dry-run --strict --config wrangler.jsonc`; `git diff --check`.

- [x] **32. Hand newer direct-read checkpoints to the next mission action atomically**
  What was built: Direct repair, diagnosis, browser-review, exploration, verification-candidate, and aggregate-verification reads now adopt their authoritative checkpoint in the same shared-service publication as the returned workspace state. Reset generation prevents delayed reads from advancing a replacement workspace. Repository fix briefs and repair workspaces return the adopted revision, and a brief fails closed if its report and route scope cross revisions.
  Acceptance: A newer direct read cannot appear under an older mission revision or trigger two observable intermediate states; the next WebMCP mutation can use the checkpoint returned by the read; a report/scope race returns `MISSION_REFRESH_UNSTABLE` with the current bounded checkpoint instead of mixed evidence; a late read after reset cannot change the new workspace; Human-only fallback, Worker/local parity, all authority boundaries, and the twenty-one tools remain unchanged.
  Verify: `bun test tests/audit-service.test.mjs tests/webmcp.test.mjs`; `bun test tests/audit-service.test.mjs tests/webmcp.test.mjs tests/sites-worker.test.mjs`; `bun test`; `bun run build`; `bunx wrangler deploy --dry-run --strict --config wrangler.jsonc`; `git diff --check`.

- [x] **33. Derive compound agent evidence from one mission revision**
  What was built: The shared service now exposes a coherent completed-results read over its existing checkpoint-bracketed restoration contract. `get_site_audit_results`, the assessment receipt, repository fix brief, and single-audit verification receipt use that boundary before combining report, repair/policy, diagnosis, browser-review, or exploration state. Isolated tool fixtures retain a compatibility fallback, while the production service always performs the coherent refresh.
  Acceptance: A fresh report cannot be combined with older browser, diagnosis, repair, or exploration caches; missing or changing mission reads fail closed without publishing a partial projection; results, receipts, and repository briefs expose the same checkpoint as their evidence; prior coherent state remains retained; Human-only fallback, contextual registration, all authority boundaries, and the twenty-one tools remain unchanged.
  Verify: `bun test tests/webmcp.test.mjs`; `bun test tests/audit-service.test.mjs tests/webmcp.test.mjs tests/sites-worker.test.mjs`; `bun test`; `bun run build`; `bunx wrangler deploy --dry-run --strict --config wrangler.jsonc`; `git diff --check`.

- [x] **34. Reconcile every mission cache before a newer direct read is published**
  What was built: Results, repair, diagnosis, browser-review, exploration collection/detail, verification-candidate, and aggregate-verification reads detect when their authoritative checkpoint is newer than the retained audit. They then reuse the complete-only checkpoint-bracketed refresh and publish the audit, repairs/policy, diagnosis, review, and explorations exactly once. Same-revision reads remain lightweight.
  Acceptance: A direct read cannot make contextual tools observe one newer family alongside older sibling caches; an incomplete reconciliation retains the prior mission and returns `MISSION_WORKSPACE_INCOMPLETE` with its bounded checkpoint; reset generation still rejects late publication; all eight direct-read families share the contract; Human-only fallback, Worker/local parity, authority boundaries, and the twenty-one tools remain unchanged.
  Verify: `bun test tests/audit-service.test.mjs tests/webmcp.test.mjs`; `bun test tests/audit-service.test.mjs tests/webmcp.test.mjs tests/sites-worker.test.mjs`; `bun test`; `bun run build`; `bunx wrangler deploy --dry-run --strict --config wrangler.jsonc`; `git diff --check`.

- [x] **35. Publish compound accepted operations as one complete state**
  What was built: Repair preparation, related-route starts, verification starts, and verification replay now retain every accepted checkpoint, audit, browser result, and refreshed report before sending one shared-service notification. Verification replay additionally requires its returned review and result to share one revision.
  Acceptance: Contextual registration cannot observe an advanced parent before a child workspace becomes active, a prepared mission twice, or a completed browser replay beside the old verification report; cross-revision replay evidence returns `MISSION_REFRESH_UNSTABLE` without changing local caches; all existing single-record mutations and activity notifications remain intact; Human-only fallback, Worker/local parity, authority boundaries, and the twenty-one tools remain unchanged.
  Verify: `bun test tests/audit-service.test.mjs`; `bun test tests/audit-service.test.mjs tests/webmcp.test.mjs tests/sites-worker.test.mjs`; `bun test`; `bun run build`; `bunx wrangler deploy --dry-run --strict --config wrangler.jsonc`; `git diff --check`.

- [x] **36. Reject contradictory compound repair-preparation responses before publication**
  Acceptance: The returned audit mission, top-level mission, audit revision, nested checkpoint revision, and authoritative top-level checkpoint describe one transition; disagreement fails closed before subscribers, Human UI state, or WebMCP output can diverge; safe revision errors retain the bounded current checkpoint; idempotent intent and all person-only authority boundaries remain unchanged.

- [ ] **37. Prove the browser-review and replay candidate in fresh supported sessions**
...
- [x] **38. Unify Lighthouse and document evidence for every audit**
  What to build: Run bounded mobile/desktop Lighthouse measurement and the public-document inspection concurrently for every root or related audit. Add one pure coverage projection and deterministic merger that retains non-overlapping rule outcomes, document metadata, headers, routes, screenshots, source failures, and provenance without letting either source erase the other.
  Acceptance: A full Lighthouse success still returns `documentProfile` and server-derived route candidates; either source may succeed independently; unavailable sources are explicit and bounded; duplicated rules are counted once; cancellation aborts both branches; provider text cannot control schemas or authority; verification comparability remains exact; no tool, dependency, Durable Object, or permission is added.
  Verify: `bun test tests/pagespeed-provider.test.mjs tests/audit-coverage-contract.test.mjs tests/sites-worker.test.mjs`; `bun run build`; `git diff --check`.

- [x] **39. Make page and bounded-site mission scope authoritative**
  What to build: Evolve audit missions to schema v2 with `scope: page | bounded-site`, a maximum-three route limit, deterministic server-issued route candidates, and read-time projection of legacy v1 missions as page scope. For bounded-site missions, derive the next exact exploration action after root evidence, retain cross-page recurrence in root priorities/checkpoints/receipts, and withhold `assessmentComplete` until the requested exploration is terminal or truthfully blocked.
  Acceptance: Page missions retain current behaviour; bounded-site input cannot become an arbitrary crawl; only observed same-site candidates may be selected; completed exploration evidence contributes to one root mission without overwriting child provenance; unavailable or failed route coverage remains partial/blocked rather than complete; Human and WebMCP starts share the schema; all twenty-one tool names and human-only authority remain unchanged.
  Verify: `bun test tests/audit-mission-contract.test.mjs tests/site-exploration-contract.test.mjs tests/mission-checkpoint-contract.test.mjs tests/assessment-receipt.test.mjs tests/audit-service.test.mjs tests/webmcp.test.mjs tests/sites-worker.test.mjs`; `bun run build`; `git diff --check`.

- [x] **40. Detect regressions inside reviewed repair verification**
  What to build: Extend the existing authoritative impact matrix with bounded baseline guardrail rows from previously passing important focus rules and retained browser journey/reflow checks on reviewed routes. Compare fresh evidence without collapsing it into the repaired exact-rule rows, and expose resolved, still-present, regression, or inconclusive outcomes through the workspace, structured receipt, and Markdown.
  Acceptance: Target-rule resolution cannot hide a newly failed retained guardrail; missing or incomparable fresh evidence remains inconclusive; newly introduced high/medium findings on reviewed routes are separately retained; browser guardrails require exact replay evidence; legacy one-row verification projects deterministically; approval, deployment, and attestation authority remain person-owned.
  Verify: `bun test tests/verification-impact-contract.test.mjs tests/repair-contract.test.mjs tests/browser-review-contract.test.mjs tests/audit-service.test.mjs tests/webmcp.test.mjs tests/sites-worker.test.mjs`; `bun run build`; `git diff --check`.

- [x] **41. Support cohesive multi-finding repair packages**
  What to build: Evolve the existing repair record and preparation intent to retain one to three diagnosed finding IDs in one reviewed repair package, with one primary finding for legacy compatibility. Union and deduplicate their evidence, repository diagnoses, server-issued verification candidates, reviewed matrix rows, implementation receipts, and aggregate verification without introducing another mission object.
  Acceptance: One-finding inputs and completed legacy repairs retain their current public shape through deterministic projections; the package cannot silently add or replace findings after preparation; every finding must belong to the same completed audit and satisfy current diagnostic readiness; one approval never broadens policy eligibility; verification resolves only when every required finding row resolves; arbitrary IDs, paths, URLs, source, and patches remain rejected at existing boundaries.
