...
The agent performs the exact check using its own browser capability and returns bounded observations. Frontmend preserves the provider and browser records separately, classifies their relationship, and finalises the audit and ranking without requesting repository data. A later fresh session can reopen the stable workspace and receive the same final assessment receipt. Only an explicit repair selection begins repository diagnosis for that frozen scope. If a repair affects multiple retained routes or strategies, the verification plan shows that impact scope before approval and proves each required target afterward.
...
- [x] **29. Unified Lighthouse and document evidence** — implemented and locally verified on 31 August 2026; every audit now retains independent categorical source coverage and bounded partial failures
- [x] **30. Authoritative page and bounded-site mission scope** — implemented and locally verified on 31 August 2026; schema v2, deterministic route candidates, cross-page recurrence, terminal coverage gating, and legacy page projection share one root mission
...
## Freeform Product Round — Unified Coverage, Site Scope, And Repair Coherence

The completed single-page evidence loop is now being deepened along five participant-approved product axes without adding a tool, dependency, Durable Object, embedded browser, or deployment authority.

1. Run Lighthouse and bounded document inspection concurrently for every audit, retain either source independently, and project explicit categorical coverage.
2. Evolve the authoritative audit mission to distinguish page scope from a maximum-three-route bounded-site scope, with legacy page projection and cross-page evidence feeding the root mission.
3. Extend the reviewed verification matrix with separately retained regression guardrails so exact-rule resolution cannot hide a newly broken baseline contract.
4. Evolve the existing repair record to support one to three diagnosed findings in one coherent reviewed package while retaining one-finding compatibility.
5. Add executable full-mission protocol evaluations across the real shared service and both HTTP runtimes, without presenting deterministic fixtures as live proof.

The unified evidence slice introduces one pure merger and coverage contract. Every audit starts independent mobile/desktop Lighthouse and live-document branches together. Full Lighthouse success now still returns bounded HTML, header, metadata, CSP, and observed-route evidence; a failed source remains a bounded `sourceFailures` record and cannot erase the completed source. Duplicate document rules are omitted from combined totals when retained Lighthouse evidence already evaluated that rule. The visible report and Markdown export expose the same categorical source coverage and limitations without inventing a confidence score.

Mission schema v2 now retains `page` or `bounded-site` as authoritative scope plus a one-to-three route limit. Legacy v1 records project deterministically as page missions. The completed root document issues deterministic candidate IDs for at most three observed same-site routes; bounded-site continuations accept those IDs rather than raw arbitrary paths. Root checkpoints name the exact exploration action, stay incomplete while route audits run, and become terminal only with complete recurrence evidence or an explicit missing/failed-coverage limitation. Completed child occurrences join root reconciliation without losing provider, rule, strategy, child audit, or path provenance. Human intake, WebMCP, the shared service, local runtime, Worker, visible mission summary, structured receipt, and Markdown receipt all use this same projection while the tool library remains at twenty-one.

Reviewed repair verification now freezes separate exact-rule, previously passing provider-rule, newly introduced high/medium finding, exact browser replay, and retained journey/reflow guardrail rows. Fresh target failures remain `still-present`; a newly failed guardrail or newly introduced retained-focus finding is `regression`; missing, blocked, or incomparable evidence remains `inconclusive`. Browser guardrails reuse the existing ordered review and require direct replay before either the single-audit or aggregate receipt unlocks. The Human matrix, verification workspace, inspector, Worker, local runtime, WebMCP, structured result, and Markdown receipt share these categorical outcomes, while legacy approved repairs still project as one row.

Repair preparation and the existing repair record now retain one to three ordered finding IDs, with the first ID remaining the legacy primary finding. Package membership freezes before repository diagnosis begins, every member is revalidated against the same completed audit, and each required diagnosis remains separately attributed. One proposal, approval, implementation receipt, and verification run cover the coherent change, while exact matrix rows retain their own finding identity and every retained browser finding receives its own ordered post-deployment replay. Multi-finding packages always require explicit person review and cannot consume or broaden delegated auto policy; single-finding and completed legacy repairs retain deterministic compatibility projections.

The full-mission evaluator exercises those contracts through the registered contextual tools and real HTTP transport rather than duplicating mission logic. One deterministic success scenario crosses bounded-site measurement, rendered evidence, a final pre-repair assessment receipt, stale-revision rejection and restoration, explicit repair selection, diagnosis, a two-finding explicit-review package, implementation metadata, site-owner attestation, and two-route verification. A separate provider/browser conflict scenario proves unavailable repository evidence leaves the final assessment receipt available while locking only repair staging. Local middleware and the public Worker adapter must return the same stable categorical outcome. The emitted summary labels its fixtures and explicitly denies live browser, repository, deployment, and production proof.
