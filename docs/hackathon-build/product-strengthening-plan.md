...
Repair preparation and the existing repair record now retain one to three ordered diagnosed finding IDs, with the first ID remaining the legacy primary finding. Package membership freezes at preparation, every member is revalidated against the same completed audit, and each required diagnosis remains separately attributed. One proposal, approval, implementation receipt, and verification run cover the coherent change, while exact matrix rows retain their own finding identity and every retained browser finding receives its own ordered post-deployment replay. Multi-finding packages always require explicit person review and cannot consume or broaden delegated auto policy; single-finding and completed legacy repairs retain deterministic compatibility projections.

The full-mission evaluator exercises those contracts through the registered contextual tools and real HTTP transport rather than duplicating mission logic. One deterministic success scenario crosses bounded-site measurement, rendered evidence, stale-revision rejection and restoration, diagnosis, assessment receipt, a two-finding explicit-review package, implementation metadata, site-owner attestation, and two-route verification. A separate provider/browser conflict scenario proves unavailable repository evidence keeps assessment receipts and repair staging locked. Local middleware and the public Worker adapter must return the same stable categorical outcome. The emitted summary labels its fixtures and explicitly denies live browser, repository, deployment, and production proof.
